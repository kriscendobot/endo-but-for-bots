/*---
description: >
  toBytes honors the byteOffset and byteLength of a subarray view: only the
...
var result = toBytes(window);

assert.sameValue(result.byteLength, 4, 'only the windowed bytes are captured');

// Verify byte values via fromBytes (which produces a plain mutable copy).
