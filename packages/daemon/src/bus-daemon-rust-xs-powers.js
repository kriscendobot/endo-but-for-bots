...
export const makeXsFilePowers = () => {
  ...
  /**
   * Binary-safe whole-file read.  The host returns an `ArrayBuffer`
   * on success (which we wrap as a `Uint8Array`) or an `"Error: ..."`
   * string on failure.  Use this rather than `readFileText` when the
   * payload is not UTF-8 — strings round-trip through XS's CESU-8
   * encoding and corrupt bytes > 0x7F.
   *
   * @type {FilePowers['readFile']}
   */
  const readFile = async path => {
    const result = hostReadFileBytes(DIR_TOKEN, toRelative(path));
    if (typeof result === 'string' && result.startsWith('Error: ')) {
      throw new Error(result);
    }
   ...
   * as `"Error: ..."` strings and are rethrown.
   *
   * @type {FilePowers['maybeReadFile']}
   */
  const maybeReadFile = async path => {
    const result = hostMaybeReadFileBytes(DIR_TOKEN, toRelative(path));
    if (result === undefined) {
      return undefined;
    }
    if (typeof result === 'string' && result.startsWith('Error: ')) {
   ...
   *
   * Uses the binary-safe {@link readFile} host call rather than
   * `readFileText` + `TextEncoder.encode`, which would corrupt any
   * byte > 0x7F (XS strings are stored as CESU-8, so non-ASCII bytes
   * round-trip incorrectly through the text path).  This is what the
   * content-addressed store relies on for ZIP archive blobs.
   *
   * @param {string} path
