...
///
/// Reads the entire file as raw bytes and returns them as an
/// ArrayBuffer.  Use this for binary file payloads — the string
/// round-trip used by [`host_read_file_text`] corrupts non-ASCII data
/// because XS treats the bytes as CESU-8.
///
/// On success returns an ArrayBuffer.  On failure returns an
...
///
/// Like [`host_read_file_bytes`] but returns `undefined` when the file
...
///
/// New callbacks must be appended (never inserted) so existing
/// snapshots can still resolve their callback indices.
...
pub unsafe fn register(machine: &crate::Machine) {
    ...
    // Binary-safe whole-file reads — return ArrayBuffer payloads so
    // bytes > 0x7F survive the host→XS boundary intact.  See
    // `host_read_file_bytes` for the rationale.
