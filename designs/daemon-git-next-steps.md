...
| 2 | [daemon-git-remotes](daemon-git-remotes.md) § Repository Bootstrap (identity residue; shape pinned in § Commit-identity boundary below) | Formula-owned commit-identity policy: `{ authorName, authorEmail }` construction-time options on `provideGit` / `provideGitClone`, guest-immutable, defaulting to today's `Endo <endo@invalid.local>` | Landed substrate only: `provideGit`, `provideGitClone` (#538), the `withGitEnvOverrides` per-invocation env seam in `@endo/git` |
...
Phases 1 → 2 → 3 are the serial critical path: Phase 3's worked loop is the milestone's exit criterion, and it needs the remote tool (Phase 1) to push and the identity boundary (Phase 2) for the pushed commits to be attributed from policy rather than from the hardcoded backend default.
