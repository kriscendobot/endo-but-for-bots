...
| ocapn-orthogonal-persistence | In Progress | `@endo/siesta` prototype landed (host, persistent CapTP tables via new `@endo/captp` `provideImport`/`provideExport` seams, journal-replay engine, sleepy workers, sturdy-ref publication). Phases 3 and 4 landed 2026-07-17: absolute-indexed journals truncated at every snapshot with snapshot+suffix restore and superseded-ref release; the XS engine rescoped onto the `xsnap` crate directly (`rust/siesta-xs-worker` + `makeXsEngine`, exit criterion met on real XS heap snapshots, skipping the broken endor bundle toolchain); durable host exports at the export-table layer (descriptions in the serialized tables record); and the worker controller (workers create and endow other workers; cross-worker links durable as `worker-import` descriptions, re-seated without waking either worker). Remaining: ses lockdown on XS, Noise transport with persisted keys |
...
| ocapn-orthogonal-persistence | M | 4-5 days | 4 | Phases 1-4 landed including the XS engine (`rust/siesta-xs-worker` on the `xsnap` crate; siesta suite green on real XS heap snapshots) and the worker controller; remaining estimate covers ses-lockdown-on-XS and the Phase 5 Noise transport wiring |
...
| M11: Rust Daemon (`endor`) (was M6) | 12-17 weeks | 62-84 weeks | Q3 to Q4 2027 (research-heavy; may run in parallel) |
