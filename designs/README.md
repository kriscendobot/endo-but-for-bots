...
`@endo/proc` standing alone; the endo/exo boundary rule is "guards
...
| platform-dimension-packages | Not Started | Explode `@endo/platform` into per-dimension endo/exo package pairs: `@endo/fs` + `@endo/exo-fs` + `@endo/fs-node` (snapshot tier), `@endo/fs-backend` + `@endo/exo-filesystem` (the extended `Filesystem`, promoting the endo-fs-backend-seam layers to a package boundary), `@endo/cas` + `@endo/cas-node` + `@endo/exo-cas` (consolidating the smeared CAS contract, powers, and `BlobRef`), and `@endo/proc`; `@endo/platform` survives as a deprecated umbrella of one-line re-export shims (per the inter-package-plain-re-exports staging) removed at next major; executes as a serial five-child orchestration |
