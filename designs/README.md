...
Chat-side control surface for the confined outbound-HTTP tier: the Value
modal's front face, for a value detected as an `HttpClient` (a `fetch` +
`allowedOrigins` remotable), becomes a control surface — an `allowedOrigins`
list and a policy-bounded request composer in the read-only view, plus the
full policy/limit/mode editors, TOFU binding table, audit log, and revoke
kill switch recovered host-side via `getHttpClientControl`; a front-face
sibling of `formula-inspector`'s read-only back face over the #661
`provideHttpClient` / `HttpClientControl` wiring, degrading silently to the
read-only view for guests and foreign clients, and flagging as its primary
open question that live control edits are session-scoped because the
`http-client` formula re-bakes its provision-time policy on daemon restart;
design PR against `llm`),
...
| [chat-http-controller-ui](chat-http-controller-ui.md) | 2026-07-15 | 2026-07-15 | Not Started |
...
| chat-http-controller-ui | Not Started | Value-modal front face becomes a control surface for a detected `HttpClient`: `allowedOrigins` list + request composer (read-only view), plus the policy/limit/mode editors, TOFU bindings, audit log, and revoke kill switch recovered via host-only `getHttpClientControl`. Sibling of `formula-inspector` over the #661 HTTP tier. Open question: live control edits are session-scoped (formula re-bakes provision-time policy on restart). |
...
| M3: Remote Access & Tools (was M1) | 18 (`gateway-package`, `daemon-docker-selfhost`, `daemon-agent-tools`, `endo-agent-tools`, `agentry-agent-builder`, `agentry-git-verb-gaps`, `agentry-git-eval-scenarios`, `daemon-mount`, `daemon-worker-import-from-mount`, `registry-capability`, `mvs-resolver`, `snapshot-mapper`, `filesystem-watchers`, `daemon-locator-terminology`, `daemon-rename-to-manager`, `daemon-xs-worker-snapshot`, `endoclaw-timer`, `endo-fetch` (was `endoclaw-network-fetch`)) | 9-13 weeks | 11-15 weeks |
