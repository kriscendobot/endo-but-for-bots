...
| [daemon-agent-tools](daemon-agent-tools.md) | 2026-03-02 | 2026-07-08 | In Progress |
...
| [daemon-worker-import-from-mount](daemon-worker-import-from-mount.md) | 2026-05-22 | 2026-07-10 | Not Started |
| [registry-capability](registry-capability.md) | 2026-06-02 | 2026-07-10 | Not Started |
| [endor-registry-transport](endor-registry-transport.md) | 2026-07-14 | 2026-07-14 | Proposed |
| [mvs-resolver](mvs-resolver.md) | 2026-06-02 | 2026-07-10 | Not Started |
| [snapshot-mapper](snapshot-mapper.md) | 2026-06-02 | 2026-07-10 | Not Started |
...
| [endoclaw-network-fetch](endoclaw-network-fetch.md) | 2026-03-03 | 2026-07-08 | In Progress |
...
flowchart TD
    ...
    subgraph Capability System
        ...
        dwixport[endor-registry-transport]
        dwimvs[mvs-resolver]
...
| daemon-worker-import-from-mount | Not Started | **Integration layer** of a four-layer stack (decomposed 2026-06-02 per kriskowal CHANGES_REQUESTED on #358; stack accepted and sequenced 2026-07-10: its § *Phased Implementation* is now the canonical dependency-ordered build plan, with Phases 1–4 the serial critical path and Phase 5 Rust drop-in plus Phase 6 XS as parallel-lane follow-ups). `makeFromPackage(mountName)` daemon-worker entry that runs a `package.json`-rooted `EndoMount` through `compartment-mapper.importLocation`; this layer carries `makeFromMount` dispatcher, worker dispatch body, CLI shape, XS bridging, architecture diagram. Sibling of `daemon-make-archive` § Phase 7 (`makeFromTree` for `compartment-map.json`-rooted trees) |
| registry-capability | Not Started | Layer 1 of 4 (accepted 2026-07-10; builds first, as stack Phase 1 with the MVS algorithm inside). `EndoRegistry` capability shape, `@registry` host special name, snapshot-vs-live-read contract, two-backend roadmap (JS reference impl ships first, Rust drop-in deferred to Phase 5). Required `registry` slot on `HostFormula` with one-shot upgrade pass for already-formulated hosts (the `@node` migration precedent). Workspace-member `RegistryResolution` entry shape pinned (bare-name key, `workspace: true`, no `integrity`) |
| mvs-resolver | Not Started | Layer 2 of 4 (accepted 2026-07-10; lands inside stack Phase 1 with registry-capability). JS reference implementation of Go-like Minimum Version Selection adapted to npm versioning (greatest mentioned minor per major; major-version coexistence admitted). Eager single-pass resolution shape (no per-import bus roundtrips). Lockfile honoring deferred as a follow-up constraint pass. Workspace-root discovery assigned to the mapper layer; the resolver consumes the `workspaceRoot` option |
| snapshot-mapper | Not Started | Layer 3 of 4 (accepted 2026-07-10; stack Phase 2, after registry-capability Phase 1). `mapSnapshot` lane in `packages/daemon/` that translates `(RegistryResolution, EndoMount)` into a `CompartmentMap` via `compartment-mapper`'s package-descriptor walker (one new extension point in `compartment-mapper`). `makeMountReadPowers` and the compartment-mapper archive-precedent layout (top-level `compartment-map.json` plus peer directories named by package; `<name>@<version>/` for registry-resolved entries, bare `<name>/` for workspace members). Owns workspace-root discovery (walk-up on the snapshot tree, threaded to `resolve` as `workspaceRoot`) |
...
| endoclaw-network-fetch | In Progress | **Strategic:** `HttpClient` with origin allowlist. Self-hosted agents need outbound HTTP; foundation for OAuth and all external integrations. Capability realized via `@endo/exo-http-client` over `@endo/http-confine` (#566); daemon `provideHttpClient` / `makeHttpTool` wiring remains ([`daemon-agent-tools`](daemon-agent-tools.md) Phase 3.6). Reference: [`trust-on-first-bind`](trust-on-first-bind.md) (TOFU-style prompt-and-pin for allowlist-bearing caps). |
...
| endor-registry-transport | Proposed | Endor/XS transport backend behind the shared `RegistryBackend` shape: three Rust host fns (`hostRegistryFetch` async-envelope, `hostGunzip`, multi-algorithm `hostHashDigest`) + `makeRegistryXsPowers` adapter reusing the JS backend verbatim. Realizes registry-capability § Two backends "Rust drop-in" for Endor. Depends on registry-capability (M3) |
...
| daemon-agent-tools | M-L | 3-4 days | 3 | Re-swept 2026-07-08: Phase 1 file tools (#614), Phase 2a/2b Shell + `makeShellTool` (#615), and local mount-bridged git tools (#616) have landed, and the HTTP capability substrate landed via #566; remaining bulk is the `makeGitRemoteTool` push tier, the `provideHttpClient` + `makeHttpTool` network wiring, the sandbox shell engine (gated on `endo-posix-sandbox`), and the Phase 4 worked loop — roughly S-M of the original M-L remains |
...
| endor-registry-transport | S-M | 3-4 days | 11 | Endor/XS transport backend behind the `RegistryBackend` shape (registry-capability § Two backends). Three Rust host fns (`hostRegistryFetch` async-envelope, `hostGunzip`, `hostHashDigest` sha512/sha256/sha1→base64) + JS `makeRegistryXsPowers` adapter reusing `makeRegistryBackend` verbatim + shared extracted `verifyIntegrity`. Node-parity test matrix. Reuses the spawn envelope pattern; no all-Rust registry fork |
...
| endoclaw-network-fetch | S-M | ~1 day | 3 | HttpClient with origin allowlist, rate/size limits; references [`trust-on-first-bind`](trust-on-first-bind.md) for the TOFU policy adapter. Capability packages landed (#566); remaining is the daemon `provideHttpClient` / `makeHttpTool` wiring, tracked at [`daemon-agent-tools`](daemon-agent-tools.md) Phase 3.6 |
...
| M4: Networking (was M2) | 7 (`ocapn-network-transport-separation`, `ocapn-tcp-for-test-extraction`, `ocapn-tcp-syrups-framing`, `cbors`, `cbor-codec`, `ocapn-noise-cryptographic-review`, `daemon-agent-network-identity`) | 4-5 weeks | 5-7 weeks |
...
| M11: Rust Daemon (`endor`) (was M6) | 3 (`endor-tui`, `endor-bus-tui`, `endor-registry-transport`) | 12-18 weeks | 14-20 weeks |
