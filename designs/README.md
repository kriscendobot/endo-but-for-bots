...
| [daemon-no-wait](daemon-no-wait.md) | 2026-07-17 | 2026-07-17 | Proposed |
...
| daemon-no-wait | Proposed | Native creation-vs-construction split: `start*` facet siblings return a durable `{ id, locator }` receipt at formula-plus-name persistence; CLI `eval`/`make`/`request` gain `--no-wait`; waiting stays the default; per PR #751 review, no daemon or tool deadlines |
...
| daemon-no-wait | M-L | ~1 week | 9 | Five slices: `startEvaluate` vertical (host+guest+guards+types+help), CLI `eval --no-wait`, caplet `startMake*` family + `make --no-wait` temp-archive rework, `startRequest` with eager response naming, conformance pinning; daemon core unchanged (`formulate` already returns `{ id, value }`) |
