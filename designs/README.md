...
flowchart TD
    ...
    subgraph Agent Capabilities
        etimer[endoclaw-timer<br/><i>SUPERSEDED</i>]
        esched[scheduler]
...
| endoclaw-timer | In Progress (Superseded by [scheduler](scheduler.md)) | **Strategic:** Core capability concern — SES removes `setTimeout`/`setInterval`; Timer is the only way agents get scheduled execution. Phase 1 prototype shipped in `packages/genie/src/interval/`. The daemon-side graduation is the [scheduler](scheduler.md) capability. |
| scheduler | Proposed | **Strategic:** Daemon-side graduation of the `packages/genie/src/interval/` prototype. Carries forward endoclaw-timer's start-to-start timing, resolve/reschedule semantics, persistence, pause/resume, revocation; adds daemon mail tick delivery, `serial-jobs`-backed coalescing, per-`Interval` pet-name granting per the daemon-capability-bank's six Design Principles. Proposed via PR #89; supersedes `endoclaw-timer`. |
...
| endoclaw-timer | S-M | 3 days | 3 | Superseded by [scheduler](scheduler.md); estimate carried forward there |
| scheduler | S-M | 3 days | 3 | Daemon-side graduation of `packages/genie/src/interval/`; new `scheduler` formula type, daemon mail tick delivery, `serial-jobs`-backed coalescing, per-`Interval` pet-name granting, `HostInterface.makeScheduler`, CLI verbs. Estimate inherits from endoclaw-timer; Phase 1 prototype already shipped |
...
| M3: Remote Access & Tools (was M1) | 16 (`gateway-package`, `daemon-docker-selfhost`, `daemon-agent-tools`, `endo-agent-tools`, `agentry-agent-builder`, `daemon-mount`, `daemon-worker-import-from-mount`, `registry-capability`, `mvs-resolver`, `snapshot-mapper`, `filesystem-watchers`, `daemon-locator-terminology`, `daemon-rename-to-manager`, `daemon-xs-worker-snapshot`, `scheduler` (supersedes `endoclaw-timer`), `endoclaw-network-fetch`) | 9-12 weeks | 11-14 weeks |
...
|--------|-----------|-----------|
| endoclaw-timer | M3 (was M1) | Superseded by [scheduler](scheduler.md). Phase 1 prototype shipped; daemon-side graduation tracked under the successor design. |
| scheduler | M3 (was M1) | **Core capability concern.** SES lockdown removes `setTimeout` and `setInterval`. The scheduler capability is the *only* mechanism for scheduled agent execution. Prerequisite for proactive messages, monitoring, reminders, and the genie-integration heartbeat. Without it, agents are purely reactive. Supersedes `endoclaw-timer` and graduates its Phase 1 prototype into a daemon-side capability. |
