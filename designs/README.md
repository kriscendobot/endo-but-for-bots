...
unsubscribed-rejection carriers, inspectable while debugging without a
production log line per hop, with no background timer, weak carrier
references, and a hardened-snapshot inspection surface; layers on the
parent's rejection-retention and `onFirstSubscribe` contracts rather
than restating them),
