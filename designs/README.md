...
Layered on the 2026-07-10 revision of [endoclaw-oauth](endoclaw-oauth.md) in M7 (Weblets and Integrations) per kriskowal review of PR #621: added holder-driven recursive partition (any `OAuth` facet holder mints monotonically-narrowed child `OAuth`/`OAuthControl` pairs, enforced by per-request conjunction along the ancestor chain so a child never out-scopes a shrinking parent, with subtree-wise revocation), composed with the existing caretaker controls; the composite is captured as the new [caretaker-attenuation](caretaker-attenuation.md) Reference pattern doc so future designs can invoke it by name; summary table, Reference count, and recently-revised prose synced.
...
[caretaker-attenuation](caretaker-attenuation.md) (added 2026-07-10;
Reference pattern doc naming the composite of the caretaker pattern (a
capability paired with a controller facet that adjusts its attenuation
dynamically) with attenuation by recursive partition (a holder mints
monotonically-narrowed child capability/control pairs and delegates
them); first full instance is endoclaw-oauth, per kriskowal review of
PR #621),
...
errors, and an explicit connector contract; revised again 2026-07-10
per review of PR #621 to add holder-driven recursive partition with
monotone narrowing, the first full instance of
...
| [daemon-git-capability](daemon-git-capability.md) | 2026-05-18 | 2026-07-06 | Proposed |
...
| [endoclaw-oauth](endoclaw-oauth.md) | 2026-03-03 | 2026-07-10 | Not Started |
| [caretaker-attenuation](caretaker-attenuation.md) | 2026-07-10 | 2026-07-10 | Reference |
...
| endoclaw-oauth | M | 4-5 days | 7 | Credential proxy exo, token injection, PKCE-loopback + device-code mint flows, shared token record, recursive partition per [caretaker-attenuation](caretaker-attenuation.md) (agent-side OAuth; distinct from gateway-oauth-bonding in M5; foundation for exo-google-sheets and siblings) |
